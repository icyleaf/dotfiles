#!/usr/bin/env bash
#
# icy-launch-webapp: middleware wrapper around omarchy-launch-webapp
#
# This is the Omarchy-agnostic entry point used by Hyprland/Waybar configs.
# Replace the implementation below to remove the Omarchy dependency.
#
# To replace: implement the desired behaviour directly here and remove
# the delegation to omarchy-launch-webapp.

if command -v "omarchy-launch-webapp" >/dev/null 2>&1; then
  exec "omarchy-launch-webapp" "$@"
else
  echo "icy-launch-webapp: 'omarchy-launch-webapp' not found. Install Omarchy or implement a replacement." >&2
  exit 127
fi
